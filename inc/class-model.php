...
class Model {
	...
	public function __construct( string $environment ) {
		$cnhsa_urls = array(
			'local'       => 'https://cnhsa.local/wp-json/cnhsa/v1',
			'development' => 'https://healthclindev.wpenginepowered.com/wp-json/cnhsa/v1',
			'staging'     => 'https://healthclinstg.wpenginepowered.com//wp-json/cnhsa/v1',
			'production'  => 'https://www.cnhsa.com/wp-json/cnhsa/v1',
		);

		$this->base_url = $cnhsa_urls[ $environment ];
	}
	...
	/**
	 * Gets the authorization header for the API request.
	 *
	 * @return string The base64 encoded authorization header.
	 */
	private function get_auth(): string {
		$pass = '';
		if ( ! defined( 'CNHSA_FEDERATION_PASS_LOCAL' ) || ! defined( 'CNHSA_FEDERATION_PASS_PROD' ) || ! defined( 'CNHSA_FEDERATION_USER' ) ) {
			$pass = '';
		}
		if ( wp_get_environment_type() === 'local' ) {
			$pass = CNHSA_FEDERATION_PASS_LOCAL;
		} elseif ( wp_get_environment_type() === 'production' ) {
			$pass = CNHSA_FEDERATION_PASS_PROD;
		} else {
			$pass = '';
		}
		if ( empty( $pass ) ) {
		...
		return base64_encode( CNHSA_FEDERATION_USER . ':' . $pass ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_encode
	}
